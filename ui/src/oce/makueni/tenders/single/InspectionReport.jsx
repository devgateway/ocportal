...
const InspectionReport = (props) => {
  ...
  const authChildren = (i) => {
    ...
    return (<div>
          ...
          && <Item label={t("inspectionReport:contractValue")}
      ...
      <div className="row">
...

        {
          ...
          && isFeatureVisible("publicView.inspectionReport.privateSectorRequests.requestDate") ?
              <Item label={t("inspectionReport:daysUntilReport")}
                    value={getNearestRequestDate(i)} col={6}/>
              : null
        }
      </div>
        {
          i.privateSectorRequests !== undefined && isFeatureVisible("publicView.inspectionReport.privateSectorRequests") ?
            <div className="row padding-top-10">
              <div className="col-md-12 sub-title">{t("inspectionReport:privateSectorRequest:pl")}
              ...
              {i.privateSectorRequests.map(psr => <div key={psr._id} className="box">
                <div className="row">
                  ...
                  {isFeatureVisible("publicView.inspectionReport.privateSectorRequests.requestDate")
                  && <Item label={t("inspectionReport:privateSectorRequest:requestDate")}
                           value={formatDate(psr.requestDate)} col={3}/>}
                </div>
              </div>)
              }
            </div> : null
  ...
  return (<AuthImplReport {...props} reportName={t("inspectionReport:label")}
