...
const PMCReport = (props) => {
  ...
  const authChildren = (i) => {
    ...
    return (<div>
          ...
          && <Item label={t("pmcReport:subcounties")} value={i.subcounties.map(item => item.label).join(', ')}
                   col={3}/>}
  ...
  const childElements = (i) => {
    ...
    return (<div key="2">
      <div className="row">
        ...
        && <Item label={t("pmcReport:projectClosureAndHandover")}
      ...
      && <React.Fragment>
        ...
        <div key="4">
          ...
          && <div className="row">
            <Item label={t("pmcReport:pmcNotes")}
                  value={i.pmcNotes && i.pmcNotes.map(item => item.text).join(', ')} col={12}/>
          </div>}
        </div>);
  }

  return (<AuthImplReport {...props} reportName={t("pmcReport:label")}
                          fmPrefix={"publicView.pmcReport"} authChildren={authChildren}
