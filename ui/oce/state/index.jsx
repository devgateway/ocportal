...
class Node {
  ...
  invalidateDep(dName) {
    schedule(() => this.parent.resolveDep(dName)
...
export class Mapping extends Node {
  ...
  depsPromise() {
    return Promise.all(
      this.deps.map(dep => this.parent.resolveDep(dep)
      .getState(this.name))
    );
  }
  ...
  subscribeTo(depName) {
    this.parent.resolveDep(depName)
    .addDep(this.name);
  }
...
export default class State extends Mapping {
  ...
  input(opts) {
    return this.field(HVar, opts);
  }
  
  substate(opts) {
    return this.field(State, opts);
  }
  
  mapping(opts) {
    return this.field(Mapping, opts);
  }
  
  remote(opts) {
    return this.field(Remote, opts);
  }
