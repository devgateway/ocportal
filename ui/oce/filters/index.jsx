...
class Filters extends translatable(Component) {
  ...
  render() {
    ...
    return <div className={cn('filters', { open })} onClick={onClick}>
      <img className="top-nav-icon" src="assets/icons/filter.svg" width="100%"
           height="100%"/> {this.t('filters:title')} <i
      className="glyphicon glyphicon-menu-down"></i>
      <div className="box row" onClick={e => e.stopPropagation()}>
        ...
        <section className="buttons col-xs-offset-4 col-xs-8">
          ...
          <button className="btn btn-default"
                  onClick={e => this.setState({ savingDashboard: true })}>
          ...
          <DropdownButton dropup id="admin-dashboard-save" title={this.t('filters:dashboard:save')}>
            <MenuItem
              onClick={e => this.saveForCurrentUser()}>{this.t('filters:dashboard:saveForAdmin')}</MenuItem>
            <MenuItem
              onClick={e => this.saveUnassigned()}>{this.t('filters:dashboard:saveUnassigned')}</MenuItem>
          </DropdownButton>
          }
          {user.loggedIn && !!dashboards.length &&
          <DropdownButton dropup id="dashboard-load-dropdown"
                          title={this.t('filters:dashboard:load')}>
